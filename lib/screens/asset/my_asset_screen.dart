...
class _MyAssetScreenState extends State<MyAssetScreen> {
  ...
  void _refresh() => _ctrl.fetchMyAssets(_auth.currentUserId);
...
class _MyAssetCard extends StatelessWidget {
  final AssetModel asset;
  const _MyAssetCard({required this.asset});

  @override
  Widget build(BuildContext context) {
    final sColor = AssetModel.statusColor(asset.status);
    final sIcon  = AssetModel.statusIcon(asset.status);
    final tColor = AssetModel.typeColor(asset.assetType);
    final tIcon  = AssetModel.typeIcon(asset.assetType);
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Row(children: [
          Container(
            width: 52, height: 52,
            decoration: BoxDecoration(color: tColor.withOpacity(0.1), borderRadius: BorderRadius.circular(16)),
            child: Icon(tIcon, color: tColor, size: 26),
          ),
          const SizedBox(width: 14),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(asset.assetName,
                style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w700, fontFamily: 'Poppins', color: AppTheme.textPrimary)),
            const SizedBox(height: 2),
            Text('${asset.brandSafe}  ·  ${asset.modelSafe}', style: AppTheme.caption),
          ])),
          Container(
            ...
            child: Row(mainAxisSize: MainAxisSize.min, children: [
              ...
              Text(AssetModel.statusLabel(asset.status),
                  style: TextStyle(fontSize: 10, fontWeight: FontWeight.w700, fontFamily: 'Poppins', color: sColor)),
            ]),
          ),
        ]),
        ...
        Row(children: [
          Expanded(child: _InfoItem(label: 'Type',   value: asset.assetType)),
          Expanded(child: _InfoItem(label: 'Code',   value: asset.assetCodeSafe)),
        ]),
        const SizedBox(height: 10),
        Row(children: [
          Expanded(child: _InfoItem(label: 'Serial', value: asset.serialNumberSafe)),
          Expanded(
            child: _InfoItem(
              label: 'Return By',
              value: asset.expectedReturnDate != null
                  ? DateFormat('dd MMM yyyy').format(asset.expectedReturnDate!)
                  : '—',
              valueColor: isOverdue
                  ? AppTheme.error
                  : daysLeft <= 3 && daysLeft >= 0
        ...
        if (asset.assignedByUserName != null && asset.assignedByUserName!.isNotEmpty) ...[
          const SizedBox(height: 10),
          Row(children: [
            Expanded(child: _InfoItem(label: 'Assigned By', value: asset.assignedByUserName!)),
            if (asset.assignedDate != null)
              Expanded(child: _InfoItem(
                label: 'Assigned On',
                value: DateFormat('dd MMM yyyy').format(asset.assignedDate!),
              )),
          ]),
        ],
        ...
        if (isOverdue) ...[
          ...
          Container(
            ...
            child: const Row(children: [
              ...
              SizedBox(width: 6),
              Expanded(child: Text(
                'Return date overdue! Please return this asset immediately.',
                style: TextStyle(fontSize: 11, fontFamily: 'Poppins', color: AppTheme.error, fontWeight: FontWeight.w500),
              )),
            ]),
          ),
        ] else if (daysLeft >= 0 && daysLeft <= 3) ...[
          ...
          Container(
            padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 6),
            decoration: BoxDecoration(
                color: AppTheme.warning.withOpacity(0.1), borderRadius: BorderRadius.circular(8)),
            child: Row(children: [
              const Icon(Icons.schedule_rounded, size: 14, color: AppTheme.warning),
              const SizedBox(width: 6),
              Text('Due in $daysLeft day${daysLeft != 1 ? 's' : ''}. Please plan return.',
                  style: const TextStyle(fontSize: 11, fontFamily: 'Poppins', color: AppTheme.warning, fontWeight: FontWeight.w500)),
            ]),
          ),
        ],
        ...
        if (asset.assignmentNote != null && asset.assignmentNote!.isNotEmpty) ...[
          ...
          Container(
            ...
            child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              const SizedBox(width: 6),
              Expanded(child: Text(asset.assignmentNote!,
                  style: const TextStyle(fontSize: 12, fontFamily: 'Poppins', color: AppTheme.textSecondary))),
            ]),
          ),
        ],
        ...
        Row(children: [
          const Spacer(),
          Row(children: [
            Text('View Details', style: TextStyle(fontSize: 12, fontFamily: 'Poppins', fontWeight: FontWeight.w600, color: tColor)),
            const SizedBox(width: 4),
            Icon(Icons.arrow_forward_ios_rounded, size: 12, color: tColor),
          ]),
        ]),
...
class _InfoItem extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    Text(label, style: const TextStyle(fontSize: 10, fontWeight: FontWeight.w600, fontFamily: 'Poppins', color: AppTheme.textHint, letterSpacing: 0.5)),
    const SizedBox(height: 3),
    Text(value, style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600, fontFamily: 'Poppins', color: valueColor ?? AppTheme.textPrimary)),
  ]);
}
    ...
    child: Padding(
      ...
      child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
        Container(
          ...
          child: const Icon(Icons.devices_other_rounded, color: AppTheme.primary, size: 52),
        ),
        const SizedBox(height: 24),
        const Text('No Assets Assigned',
            style: TextStyle(fontSize: 20, fontWeight: FontWeight.w700, fontFamily: 'Poppins', color: AppTheme.textPrimary)),
        const SizedBox(height: 10),
        const Text(
          'You currently have no assets assigned.\nContact your admin for asset allocation.',
          textAlign: TextAlign.center,
          style: TextStyle(fontFamily: 'Poppins', color: AppTheme.textSecondary, fontSize: 13, height: 1.6),
        ),
        const SizedBox(height: 28),
        ElevatedButton.icon(
          onPressed: onRetry,
          icon: const Icon(Icons.refresh_rounded, size: 18),
          label: const Text('Refresh', style: TextStyle(fontFamily: 'Poppins', fontWeight: FontWeight.w600)),
          style: ElevatedButton.styleFrom(
            backgroundColor: AppTheme.primary,
            foregroundColor: Colors.white,
