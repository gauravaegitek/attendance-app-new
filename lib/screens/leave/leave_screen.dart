...
class LeaveScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // ✅ ensure LeaveController registered once
    if (!Get.isRegistered<LeaveController>()) {
      Get.put(LeaveController());
    }

    // ✅ Admin Panel se aaya hai?
    final args = Get.arguments;
    final fromAdminPanel = args is Map && args['adminPanel'] == true;

    return Obx(() {
      final isAdmin = auth.isAdmin;

      // ✅ Admin Panel → seedha All Leaves screen (no tabs)
      if (isAdmin && fromAdminPanel) {
        return Scaffold(
          backgroundColor: AppTheme.background,
          appBar: AppBar(
            backgroundColor: AppTheme.cardBackground,
      ...
      child: Column(
        ...
        children: [
          ...
          Obx(() {
            if (ctrl.fromDate.value != null && ctrl.toDate.value != null) {
              final days =
                  ctrl.toDate.value!.difference(ctrl.fromDate.value!).inDays +
                      1;
              return Padding(
                padding: const EdgeInsets.only(top: 8),
                child: Row(
          ...
          Obx(
            () => SizedBox(
              ...
              child: ElevatedButton(
                onPressed:
                    ctrl.isApplying.value ? null : () => ctrl.applyLeave(),
                style: ElevatedButton.styleFrom(
                  backgroundColor: AppTheme.primary,
                  padding: const EdgeInsets.symmetric(vertical: 16),
                  shape: RoundedRectangleBorder(
...
class _MyLeavesTabState extends State<_MyLeavesTab>
  ...
  void _confirmCancel(BuildContext context, LeaveController ctrl, int leaveId) {
    showDialog(
      context: context,
      builder: (_) => AlertDialog(
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)),
          ...
          if (!isAdmin && leave.status.toLowerCase() == 'pending') ...[
            ...
            GestureDetector(
              ...
              child: Row(
                mainAxisAlignment: MainAxisAlignment.center,
                children: const [
                  Icon(Icons.close_rounded, color: AppTheme.error, size: 16),
                  SizedBox(width: 6),
                  Text(
                    'Cancel Leave',
                    style: TextStyle(
        ...
        itemBuilder: (_, i) {
          ...
          return GestureDetector(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                color:
                    isSelected ? AppTheme.primary : AppTheme.cardBackground,
                borderRadius: BorderRadius.circular(20),
                border: Border.all(
