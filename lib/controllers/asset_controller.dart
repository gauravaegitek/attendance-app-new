...
class AssetController extends GetxController {
  ...
  // ── Fetch: My Assets (only currently assigned) ────────────────────────
  Future<void> fetchMyAssets(int userId) async {
    isLoading.value = true;
    try {
      final uri = Uri.parse('$_base${AppConstants.assetListEndpoint}')
      ...
      if (res.statusCode == 200) {
        final data = jsonDecode(res.body);
        final list = (data['data'] ?? []) as List;
        // Client-side filter: only assigned status
        myAssets.assignAll(
          list
              .map((e) => AssetModel.fromJson(e))
              .where((a) => a.status.toLowerCase() == AssetStatus.assigned)
              .toList(),
        );
      }
    } catch (e) {
  ...
  Future<void> fetchMaintenanceList({
    ...
    try {
      ...
      if (res.statusCode == 200) {
        final data = jsonDecode(res.body);
        final list = (data['data'] ?? []) as List;
        maintenance.assignAll(list.map((e) => MaintenanceModel.fromJson(e)));
      }
    } catch (e) {
      debugPrint('fetchMaintenanceList error: $e');
      ResponseHandler.showError(
  ...
  // ── Open Maintenance Request (Step 1: open) ───────────────────────────
  Future<bool> openMaintenanceRequest({
    required int    assetId,
    required String maintenanceType,
    String?         issueDescription,
  }) async {
    ...
    try {
      ...
      debugPrint('openMaintenanceRequest body: $body');
      final res = await http
          .post(
            Uri.parse('$_base${AppConstants.assetMaintenanceStartEndpoint}'),
            headers: _authHeaders,
            body: body,
          )
          .timeout(const Duration(milliseconds: AppConstants.connectTimeout));
      debugPrint('openMaintenanceRequest status: ${res.statusCode}');
      debugPrint('openMaintenanceRequest body  : ${res.body}');
      final data = jsonDecode(res.body);
      if (res.statusCode == 200 || res.statusCode == 201) {
        ResponseHandler.showSuccess(
            apiMessage: data['message'] ?? '',
            fallback: 'Maintenance request opened!');
        await fetchAssets();
        await fetchSummary();
        return true;
      }
      ResponseHandler.showError(
          apiMessage: data['message'] ?? '',
          fallback: 'Failed to open maintenance request');
      return false;
    } catch (e) {
      debugPrint('openMaintenanceRequest error: $e');
      ResponseHandler.showError(apiMessage: '', fallback: 'Error: $e');
      return false;
    } finally {
      isSubmitting.value = false;
  ...
  // ── Start Maintenance (Step 2: open → in_progress) ────────────────────
  Future<bool> startMaintenance({
    required int    assetId,
    required String maintenanceType,
    String?         vendorName,
  ...
  // ── Complete Maintenance (Step 3: mark done) ──────────────────────────
  Future<bool> completeMaintenance({
    required int   assetId,
    double?        cost,
    String?        resolutionNote,
