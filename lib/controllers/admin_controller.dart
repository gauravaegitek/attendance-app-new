...
class AdminController extends GetxController {
  ...
  // ✅ Internal flag — fetchRoles ke time selectedRole change se summary na chale
  ...
  void onInit() {
    super.onInit();
    fetchRoles();
    // ✅ fetchAdminSummary yahan se nahi call hogi — sirf search button pe
  }
  ...
  Future<void> fetchRoles() async {
    ...
    try {
      ...
      isLoadingRoles.value = false;
      _rolesInitializing = false; // ✅ unlock
    }
  }

  // =================== ADMIN SUMMARY ===================
  Future<void> fetchAdminSummary() async {
    if (_rolesInitializing) return; // ✅ initialization ke time ignore karo
    final diff = toDate.value.difference(fromDate.value).inDays;
    if (diff > 31) {
      AppUtils.showError('Unable to load summary. Date range cannot exceed 31 days.');
      return;
    ...
    try {
      ...
      // Date descending, same date mein inTime descending
      records.sort((a, b) {
        final dateCompare = b.attendanceDate.compareTo(a.attendanceDate);
        if (dateCompare != 0) return dateCompare;
        final aTime = a.inTime ?? '';
