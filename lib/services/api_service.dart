...
// ✅ Conditional imports — dart:html on web, dart:io on mobile
import 'dart:convert';
import 'dart:io';                          // ✅ Needed for File/Directory on mobile
import 'package:flutter/foundation.dart';
import 'package:get/get.dart';
import 'package:http/http.dart' as http;
...
import '../models/models.dart';
import '../models/daily_task_model.dart';
import '../models/document_model.dart';       // ✅ NEW
import '../models/holiday_model.dart';
import '../models/leave_model.dart';
import '../models/login_history_model.dart';
import '../models/notification_model.dart';
...
class ApiService {
  ...
  static Future<bool> _exportForRole({
    ...
    if (response.statusCode == 200) {
      if (!kIsWeb) {
        // ✅ dart:io is safe on mobile/desktop only
        final tempDir = Directory.systemTemp;
        final file    = File('${tempDir.path}/admin_summary_$role.pdf');
        await file.writeAsBytes(response.bodyBytes);
      }
  ...
  static Future<ApiResponse> uploadDocument({
    ...
    required XFile  file,               // ✅ XFile works on web + mobile
  }) async {
    try {
      final bytes    = await file.readAsBytes(); // ✅ web-compatible
      final fileName = file.name;

      final request = http.MultipartRequest(
        'POST',
        Uri.parse('$_base${AppConstants.documentUploadEndpoint}'),
      );
      ...
      request.files.add(
        http.MultipartFile.fromBytes(  // ✅ fromBytes — no dart:io needed
          'File',
          bytes,
          filename: fileName,
        ),
  ...
  static Future<List<DocumentModel>> getDocumentList({int? employeeId}) async {
    try {
      final params = <String, String>{};
      if (employeeId != null) params['EmployeeId'] = employeeId.toString();

      final uri =
          Uri.parse('$_base${AppConstants.documentListEndpoint}')
              .replace(queryParameters: params.isNotEmpty ? params : null);

      debugPrint('getDocumentList URI: $uri');

      final response = await http
          .get(uri, headers: _authHeaders)
          .timeout(const Duration(milliseconds: AppConstants.connectTimeout));

      debugPrint('getDocumentList status: ${response.statusCode}');
      debugPrint('getDocumentList body  : ${response.body}');
      ...
      debugPrint('getDocumentList error: $e');
      return [];
    }
  }

  static Future<bool> downloadDocumentPdf({required int employeeId}) async {
    try {
      final uri =
          Uri.parse('$_base${AppConstants.documentDownloadEndpoint}')
              .replace(queryParameters: {'employeeId': employeeId.toString()});

      debugPrint('downloadDocumentPdf URI: $uri');

      final response = await http
          .get(uri, headers: _authHeaders)
          .timeout(const Duration(milliseconds: AppConstants.connectTimeout));

      debugPrint('downloadDocumentPdf status: ${response.statusCode}');

      _checkSession(response);

      if (response.statusCode == 200) {
        debugPrint('PDF received: ${response.bodyBytes.length} bytes');
        // ✅ Web: just logs — actual web download requires dart:html
        //    in a separate conditional-import file (add if needed)
        // ✅ Mobile/Desktop: dart:io is safe when kIsWeb == false
        debugPrint(kIsWeb
            ? 'Web download: use url_launcher or separate dart:html helper'
            : 'PDF ready — save via path_provider on mobile');
        return true;
      }
      return false;
    } catch (e) {
      debugPrint('downloadDocumentPdf error: $e');
      return false;
    }
  }

  static Future<ApiResponse> deleteDocument({required int documentId}) async {
    try {
      final uri =
          Uri.parse('$_base${AppConstants.documentRemoveEndpoint}')
              .replace(queryParameters: {'documentId': documentId.toString()});

      debugPrint('deleteDocument URI: $uri');

      final response = await http
          .delete(uri, headers: _authHeaders)
          .timeout(const Duration(milliseconds: AppConstants.connectTimeout));

      debugPrint('deleteDocument status: ${response.statusCode}');
      debugPrint('deleteDocument body  : ${response.body}');
  ...
  static Future<ApiResponse> verifyDocument({
    ...
    try {
      ...
      debugPrint('verifyDocument body: $body');

      final response = await http
          .put(
            Uri.parse('$_base${AppConstants.documentVerifyEndpoint}'),
            headers: _authHeaders,
            body: body,
          )
          .timeout(const Duration(milliseconds: AppConstants.connectTimeout));

      debugPrint('verifyDocument status: ${response.statusCode}');
      debugPrint('verifyDocument body  : ${response.body}');
  ...
  static Future<DocumentSummaryModel?> getDocumentSummary({
    String? fromDate,
    String? toDate,
  }) async {
    try {
      ...
      final uri =
          Uri.parse('$_base${AppConstants.documentSummaryEndpoint}')
              .replace(queryParameters: params.isNotEmpty ? params : null);

      debugPrint('getDocumentSummary URI: $uri');

      final response = await http
          .get(uri, headers: _authHeaders)
          .timeout(const Duration(milliseconds: AppConstants.connectTimeout));

      debugPrint('getDocumentSummary status: ${response.statusCode}');
      debugPrint('getDocumentSummary body  : ${response.body}');

      _checkSession(response);

      if (response.statusCode == 200) {
        final body = jsonDecode(response.body);
        // ✅ API wraps summary in body['data'] — extract it first
        if (body is Map<String, dynamic> && body['data'] is Map<String, dynamic>) {
          return DocumentSummaryModel.fromJson(body['data'] as Map<String, dynamic>);
        }
      }
      return null;
    } catch (e) {
      debugPrint('getDocumentSummary error: $e');
      return null;
    }
  }
